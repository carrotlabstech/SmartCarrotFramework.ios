...
internal extension Interpreter {
  /// Options for configuring the `Interpreter`.
    ...
    func initWeightTensor(weightTensor: [Int]) {
        let int32Pointer = UnsafeMutablePointer<Int32>.allocate(capacity: weightTensor.count)
        
        for i in 0...weightTensor.count - 1 {
            (int32Pointer + i).pointee = Int32(weightTensor[i])
        }
        TfLiteSetWeightsTensor(int32Pointer)
    }
  }
}
...
internal typealias InterpreterOptions = Interpreter.Options
